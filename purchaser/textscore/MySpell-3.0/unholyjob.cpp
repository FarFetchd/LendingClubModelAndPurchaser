(ENTIRETY OF CODE FROM MySpell-3.0 LIB GOES HERE)
...
//UPDATE SEPT 2016: " - " and " / " and " & " should be considered as a single space.
double properJobTitle(std::string input, MySpell* theSpeller)
{
    ...
    if(input.find(" / ") != string::npos)
		input.replace(input.find(" / "), 3, string(" "));
    if(input.find(" - ") != string::npos)
		input.replace(input.find(" - "), 3, string(" "));
    if(input.find(" & ") != string::npos)
		input.replace(input.find(" & "), 3, string(" "));
...
Rcpp::NumericVector properjobtitle_internal(
...
// [[Rcpp::export]]
Rcpp::NumericVector properjobtitleprod(std::vector<std::string> input)
{
    MySpell* theSpeller = new MySpell("purchaser/textscore/MySpell-3.0/en_US.aff",
	                                  "purchaser/textscore/MySpell-3.0/en_US.dic");
    return properjobtitle_internal(input, theSpeller);
}

// [[Rcpp::export]]
Rcpp::NumericVector properjobtitledev(std::vector<std::string> input)
{
    MySpell* theSpeller = new MySpell(
        "purchaser/textscore/MySpell-3.0/en_US.aff",
	    "purchaser/textscore/MySpell-3.0/en_US.dic");
    return properjobtitle_internal(input, theSpeller);
}
